...
/*
Plugin Name: Worker Core Plugin
Plugin URI: http://www.themecanon.com
Description: Core functionality plugin for Worker theme by Theme Canon.
Version: 1.1
Author: ThemeCanon
...
***************************************/
	...
	//front end includes
	add_action('wp_enqueue_scripts','worker_core_plugin_load_to_front');
	function worker_core_plugin_load_to_front() {
	}

	//back end includes
	add_action('admin_enqueue_scripts', 'worker_core_plugin_load_to_back');  //this was changed to admin_enqueue_scripts from action hook admin_footer. Let's see if it makes a difference
	function worker_core_plugin_load_to_back() {
		...
		wp_enqueue_script('worker_core_plugin_backend_scripts', plugins_url('', __FILE__ ) . '/js/backend_scripts.js', array(), false, true);

		//style (css)	
		wp_enqueue_style('worker_core_plugin_style', plugins_url('', __FILE__ ) . '/css/style.css');
...
***************************************/
	...
	add_action('after_setup_theme', 'worker_core_plugin_localization_setup');
	function worker_core_plugin_localization_setup() {
	    load_plugin_textdomain('loc_worker_core_plugin', false,  dirname( plugin_basename( __FILE__ ) ) . '/lang/');
	}
