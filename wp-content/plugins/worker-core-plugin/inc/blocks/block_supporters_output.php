<?php
	...
	function block_supporters_output ($params) {
		...
		// FAILSAFE DEFAULTS
		if (!isset($links_open_window)) { $links_open_window = "unchecked"; }
		...
		$target_class = ($links_open_window == "checked") ? "target=_blank" : "";
				  				...
				  				if ( ($repeat == "checked") && (count($img) > 0) ) {
				  					...
				  					for ($n = 0; $n < $times_to_repeat; $n++) {  
						  				for ($i = 0; $i < count($img); $i++) { 
						  					...
						  					echo "<li>";
						  					if (!empty($img[$i]['link_url'])) { printf('<a href="%s" %s>', esc_url($img[$i]['link_url']), esc_attr($target_class)); }
