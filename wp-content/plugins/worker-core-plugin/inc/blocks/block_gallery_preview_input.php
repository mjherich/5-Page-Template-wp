<?php
	function block_gallery_preview_input ($passed_vars) {
		...
		$block_defaults = array( 
			...
			'meta_text' 					=> "January 1st, 2014",
			'button_text' 					=> "View Gallery",
			'description' 					=> "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur id neque urna. Morbi fringilla risus non risus ornare elementum.",
			'source' 						=> "",
			'num_columns' 					=> 3,
		);
		...
		?>
			...
			<!--  BLOCK CONTENT -->
				...
				<!-- BLOCK TAB: GENERAL -->
					...
					<!-- TEXT INPUT -->
						<div class="option">
							<label><?php _e("Meta text", "loc_worker_core_plugin"); ?></label>
							<input class='block_option' type='text' name='canon_options_pagebuilder[blocks][<?php echo esc_attr($index); ?>][meta_text]' value="<?php if (isset($params['meta_text'])) echo htmlspecialchars($params['meta_text']); ?>">
						</div>
					...
					<!-- TEXT INPUT -->
						<div class="option">
							...
							<input class='block_option' type='text' name='canon_options_pagebuilder[blocks][<?php echo esc_attr($index); ?>][button_text]' value="<?php if (isset($params['button_text'])) echo htmlspecialchars($params['button_text']); ?>">
						</div>
						
					<!-- TEXT INPUT -->
						<div class="option">
							<label><?php _e("Button URL", "loc_worker_core_plugin"); ?></label>
							<input class='block_option' type='text' name='canon_options_pagebuilder[blocks][<?php echo esc_attr($index); ?>][button_url]' value="<?php if (isset($params['button_url'])) echo htmlspecialchars($params['button_url']); ?>">
						</div>
