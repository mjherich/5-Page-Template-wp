...
/**************************************
WIDGET: worker_more_posts
***************************************/

	add_action('widgets_init', 'register_widget_worker_more_posts' );
	function register_widget_worker_more_posts () {
		register_widget('worker_more_posts');	
	}

	class worker_more_posts extends WP_Widget {
		...
		***************************************/
		function __construct () {
				...
				$widget_ops = array(
					'classname' => 'worker_more_posts', 								
					'description' => __('Display more posts', "loc_worker_widgets_plugin")	 				
				);
				$control_ops = array(
					'width' => 300, 
					'height' => 350, 
					'id_base' => 'worker_more_posts' 														
				);

				$this->WP_Widget('worker_more_posts',__('Worker: More Posts', "loc_worker_widgets_plugin")	, $widget_ops, $control_ops );	
		}
		...
		***************************************/
		function form($instance) {
			...
			//defaults
			$defaults = array( 
				'title' 			=> __('More posts', "loc_worker_widgets_plugin")	,
			...
			?>
				...
				<p>
					<label for="<?php echo $this->get_field_id('title'); ?> "><?php _e("Title", "loc_worker_widgets_plugin"); ?>:	 </label><br>
				...
				<p>
					<label for="<?php echo $this->get_field_id('posts_from'); ?> "><?php _e("What to show", "loc_worker_widgets_plugin"); ?>:	 </label><br>
					<select id="<?php echo $this->get_field_id('posts_from'); ?>" name="<?php echo $this->get_field_name('posts_from'); ?>"> 
		     			<option value="latest_posts" <?php if (isset($posts_from)) {if ($posts_from == "latest_posts") echo "selected='selected'";} ?>><?php _e("Latest posts", "loc_worker_widgets_plugin"); ?>	</option> 
	 					<option value="random_posts" <?php if (isset($posts_from)) {if ($posts_from == "random_posts") echo "selected='selected'";} ?>><?php _e("Random posts", "loc_worker_widgets_plugin"); ?>	</option> 
		     			
		     			<option value=""><hr></option> 

		     			<option value="popular_views" <?php if (isset($posts_from)) {if ($posts_from == "popular_views") echo "selected='selected'";} ?>><?php _e("Popular posts by views", "loc_worker_widgets_plugin"); ?>	</option> 
	 					<option value="popular_comments" <?php if (isset($posts_from)) {if ($posts_from == "popular_comments") echo "selected='selected'";} ?>><?php _e("Popular posts by comments", "loc_worker_widgets_plugin"); ?>	</option> 
				...
				<p>
					<label for="<?php echo $this->get_field_id('display_style'); ?> "><?php _e("Display style", "loc_worker_widgets_plugin"); ?>	: </label><br>
					<select id="<?php echo $this->get_field_id('display_style'); ?>" name="<?php echo $this->get_field_name('display_style'); ?>"> 
		     			<option value="images_to_posts" <?php if (isset($display_style)) {if ($display_style == "images_to_posts") echo "selected='selected'";} ?>><?php _e("Images linking to posts", "loc_worker_widgets_plugin"); ?>	</option> 
		     			<option value="images_to_lightbox" <?php if (isset($display_style)) {if ($display_style == "images_to_lightbox") echo "selected='selected'";} ?>><?php _e("Images linking to lightbox", "loc_worker_widgets_plugin"); ?>	</option> 
	 					<option value="text" <?php if (isset($display_style)) {if ($display_style == "text") echo "selected='selected'";} ?>><?php _e("Text", "loc_worker_widgets_plugin"); ?>	</option> 
					</select> 
				</p>

				<p>
					<label for='<?php echo $this->get_field_id('num_posts'); ?>'><?php _e("Number of posts", "loc_worker_widgets_plugin"); ?>	: </label><br>
					<input 
						style='width: 40px;'
						type='number' 
						min='1'
				...
				<p>
					<label for='<?php echo $this->get_field_id('num_columns'); ?>'><?php _e("Number of image columns", "loc_worker_widgets_plugin"); ?>	: </label><br>
					<input 
						style='width: 40px;'
						type='number' 
						min='1'
