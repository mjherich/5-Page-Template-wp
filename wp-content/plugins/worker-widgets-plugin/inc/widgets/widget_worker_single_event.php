...
/**************************************
WIDGET: worker_single_event
***************************************/

	add_action('widgets_init', 'register_widget_worker_single_event' );
	function register_widget_worker_single_event () {
		register_widget('worker_single_event');	
	}

	class worker_single_event extends WP_Widget {
		...
		***************************************/
		function __construct () {
				...
				$widget_ops = array(
					'classname' => 'worker_single_event', 								
					'description' => __('Display a single event', "loc_worker_widgets_plugin")	 				
				);
				$control_ops = array(
					'width' => 300, 
					'height' => 350, 
					'id_base' => 'worker_single_event' 														
				);

				$this->WP_Widget('worker_single_event', __('Worker: Single Event', "loc_worker_widgets_plugin"), $widget_ops, $control_ops );	
		}
		...
		***************************************/
		function form($instance) {
			...
			//defaults
			$defaults = array( 
				'title' 			=> __('Event', "loc_worker_widgets_plugin"),
				'excerpt_length' 	=> 650,
			);
			...
			?>
				...
				<?php 
					...
					if (!class_exists('Tribe__Events__Main')) {
						echo '<p>';
						_e("<i><strong>WARNING:</strong> This block requires <strong>The Events Calendar</strong> plugin. The required plugin could not be found. Please go to plugins and install/activate the required plugin!</i>", "loc_worker_widgets_plugin");
						echo '</p>';
					} else  {
					?>
					...
					<!-- DYNAMIC SELECT -->
						<?php 
						 ...
						 ?>
						<p>
							<label><?php _e("Select event", "loc_worker_widgets_plugin"); ?>: </label><br>
				...
				<p>
					<label for='<?php echo esc_attr($this->get_field_id('excerpt_length')); ?>'><?php _e("Automatic Excerpt Size", "loc_worker_widgets_plugin"); ?>	: </label><br>
					<input 
						style='width: 80px;'
						type='number' 
						min='0'
		...
		***************************************/
		function widget($args, $instance) {
			...
			if (function_exists('icl_translate') && function_exists('icl_register_string')) {
				...
				// VERSION < 3.3
				icl_register_string ('loc_worker_widgets_plugin', "$widget_id-widget[cost]", $cost);

				$cost = icl_translate('loc_worker_widgets_plugin', "$widget_id-widget[cost]", $cost);
			
			} elseif (class_exists('SitePress')) {

				// VERSION > v3.3
				do_action('wpml_register_single_string', 'loc_worker_widgets_plugin', "$widget_id-widget[cost]", $cost);
				
				$cost = apply_filters('wpml_translate_single_string', $cost, 'loc_worker_widgets_plugin', "$widget_id-widget[cost]");
            	...
            	<div class="clearfix tribe-events-tcblock">
					...
					<div class="tribe-events-list-event-description tribe-events-content description entry-summary">
						...
						<?php 
							...
							// read more
							printf('<a href="%s" class="tribe-events-read-more" rel="bookmark">%s &raquo;</a>', esc_url(tribe_get_event_link($event_ID)), esc_attr(__('Find out more', "loc_worker_widgets_plugin")) );
