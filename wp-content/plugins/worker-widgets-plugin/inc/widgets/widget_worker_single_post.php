...
/**************************************
WIDGET: worker_single_post
***************************************/

	add_action('widgets_init', 'register_widget_worker_single_post' );
	function register_widget_worker_single_post () {
		register_widget('worker_single_post');	
	}

	class worker_single_post extends WP_Widget {
		...
		***************************************/
		function __construct () {
				...
				$widget_ops = array(
					'classname' => 'worker_single_post', 								
					'description' => __('Displays a single post', "loc_worker_widgets_plugin")	 				
				);
				$control_ops = array(
					'width' => 300, 
					'height' => 350, 
					'id_base' => 'worker_single_post' 														
				);

				$this->WP_Widget('worker_single_post', __('Worker: Single Post', "loc_worker_widgets_plugin"), $widget_ops, $control_ops );	
		}
		...
		***************************************/
		function form($instance) {
			...
			?>
				...
				<p>
					<label for="<?php echo $this->get_field_id('orderby'); ?> "><?php _e("Order posts by", "loc_worker_widgets_plugin"); ?>:</label><br>
					<select name="<?php echo $this->get_field_name('orderby'); ?>"> 
		     			<option value="post_date" <?php if (isset($orderby)) {if ($orderby == "date") echo "selected='selected'";} ?>><?php _e("Date", "loc_worker_widgets_plugin"); ?></option> 
		     			<option value="title" <?php if (isset($orderby)) {if ($orderby == "title") echo "selected='selected'";} ?>><?php _e("Title", "loc_worker_widgets_plugin"); ?></option> 
					</select> 

					<select name="<?php echo $this->get_field_name('order'); ?>"> 
		     			<option value="DESC" <?php if (isset($order)) {if ($order == "DESC") echo "selected='selected'";} ?>><?php _e("Descending", "loc_worker_widgets_plugin"); ?></option> 
		     			<option value="ASC" <?php if (isset($order)) {if ($order == "ASC") echo "selected='selected'";} ?>><?php _e("Ascending", "loc_worker_widgets_plugin"); ?></option> 
					</select> 
				</p>

				<p><i>(<?php _e("Takes effect on save", "loc_worker_widgets_plugin"); ?>)</i></p>
				<hr>
				...
				<p>	
					<label for="<?php echo $this->get_field_id('post_ID'); ?> "><?php _e("Display post", "loc_worker_widgets_plugin"); ?>:</label><br>
				...
				<p>
					<label for='<?php echo $this->get_field_id('excerpt_length'); ?>'><?php _e("Excerpt length", "loc_worker_widgets_plugin"); ?>	: </label><br>
					<input 
						style='width: 80px;'
						type='number' 
						min='0'
		...
		***************************************/
		function widget($args, $instance) {
            ...
            <a href="<?php echo get_permalink($this_post->ID); ?>" class="more"><?php _e("More", "loc_worker_widgets_plugin"); ?></a>
