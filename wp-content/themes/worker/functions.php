...
***************************************/
	...
	if (!function_exists("canon_load_to_front")) { function canon_load_to_front() {	
		...
		//external scripts
		if (is_page()) wp_enqueue_script('canon-isotope', get_template_directory_uri(). '/js/jquery.isotope.js', array(), false, true);
		...
		wp_enqueue_script('canon-fancybox-thumbs', get_template_directory_uri() . '/js/fancybox/source/helpers/jquery.fancybox-thumbs.js', array('canon-fancybox-core'), false, true);
		...
		if (is_page()) wp_enqueue_script('canon-owl-carousel', get_template_directory_uri(). '/js/owl-carousel/owl.carousel.min.js', array(), false, true);
		...
		if (is_page()) wp_enqueue_style('canon-isotope-style', get_template_directory_uri(). '/css/isotope.css');
		...
		if (is_page()) wp_enqueue_style('canon-owl-carousel-style', get_template_directory_uri(). '/js/owl-carousel/owl.carousel.css');
		if (is_page()) wp_enqueue_style('canon-owl-carousel-theme-style', get_template_directory_uri(). '/js/owl-carousel/owl.theme.css');
	...
	if (!function_exists("canon_load_to_back")) { function canon_load_to_back() {	
		...
		//external scripts
		if ( strpos(get_current_screen()->id, 'canon_options_advanced') !== false ) wp_enqueue_script('canon-ace', get_template_directory_uri(). '/js/jquery.ace.js', array(), false, true);
		if ( strpos(get_current_screen()->id, 'canon_options_advanced') !== false ) wp_enqueue_script('canon-ace-core', get_template_directory_uri(). '/js/ace/ace.js', array(), false, true);
		if ( strpos(get_current_screen()->id, 'canon_options_advanced') !== false ) wp_enqueue_script('canon-ace-theme-chrome', get_template_directory_uri(). '/js/ace/theme-chrome.js', array(), false, true);
		if ( strpos(get_current_screen()->id, 'canon_options_advanced') !== false ) wp_enqueue_script('canon-ace-mode-css', get_template_directory_uri(). '/js/ace/mode-css.js', array(), false, true);
...
***************************************/
	...
	***************************************/
		...
		// construction mode reminder
		if ($canon_options['use_construction_mode'] == "checked") { add_action('admin_notices','canon_construction_mode_reminder'); }
	...
	***************************************/
		...
		add_image_size( 'portfolio_isotope_x2', 900, 600, true);
...
***************************************/
	...
	if (!function_exists("timeline_load_more")) { function timeline_load_more() {	
...
***************************************/
	...
	if (!function_exists("canon_filter_wp_title")) { function canon_filter_wp_title( $title, $sep ) {	
		...
		if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
			$title .= " $sep " . sprintf('%s %s', __("Page", "loc_canon"), max( $paged, $page ) );
		}
...
***************************************/
	...
	if (!function_exists("canon_construction_mode_reminder")) { function canon_construction_mode_reminder() {	
		printf('<div class="error"><p>%s</p></div>', __('Construction mode is on - remember that only logged-in users will be able to see your site pages. Go to <i>Settings > General > Construction Mode</i> to disable.','loc_canon'));
	}}
