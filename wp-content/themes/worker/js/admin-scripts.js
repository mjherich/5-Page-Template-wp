...
*************************************************************/
	...
	jQuery(document).ready(function($) {
		...
		// bouncer
		if ($('#final_call_css').size() == 0) { return; }

		// init
		$('#final_call_css').ace({ theme: 'chrome', lang: 'css' });

		// get the ace editor instance
		var aceObject = $('#final_call_css').data('ace');
  		var aceInstance = aceObject.editor.ace;
...
*************************************************************/
	...
	jQuery(document).ready(function($) {
		...
		if ( 
			...
			var HTMLString = '';
			HTMLString += '<p><strong>Plugins that come bundled with the theme cannot be updated automatically as long as the old versions are still active/installed.</strong></p>';
			HTMLString += '<p><strong>Please deactivate and delete the old versions of the plugins that need to be updated and then follow the on-screen instructions in this prompt to install the updated versions.</strong></p>';
			if (pluginUpdateNoticeHTML.indexOf('Revolution Slider') > -1) { HTMLString += '<p><strong>Remember to backup/export Revolution Slider content (sliders) before deletion.</strong></p>'; }
...
<ul class="ul_sortable" data-split_index="" data-placeholder"">
	<li>
...
data-placeholder sets css class of placholder. If styles applied to this class does not show try to add more selectors to make css declaration more specific.
*************************************************************/
